...
public class PopularKeywordQueryService {
  ...
  public PopularKeywordResponse getPopularKeywords(LocalDateTime from, LocalDateTime to, int limit) {
    ...
    // 이전 기간의 인기 검색어 조회
    List<KeywordStats> previousKeywords = statsRepository.getPopularKeywords(previousFrom, previousTo, limit * 2);
  ...
  public PopularKeywordResponse getTrendingKeywords(LocalDateTime from, LocalDateTime to, int limit) {
    log.info("급등검색어 조회 - 기간: {} ~ {}, 제한: {}", from, to, limit);
    
    LocalDateTime previousFrom = from.minusDays(to.toLocalDate().toEpochDay() - from.toLocalDate().toEpochDay());
    LocalDateTime previousTo = from;
    ...
    // 이전 기간의 순위를 Map으로 저장
    Map<String, Integer> previousRanks = previousKeywords.stream()
    ...
    List<PopularKeywordResponse.KeywordStats> trendingKeywords = currentKeywords.stream()
        .map(current -> {
          int previousRank = previousRanks.getOrDefault(current.getKeyword(), lastPreviousRank);
  ...
  private List<PopularKeywordResponse.KeywordStats> calculateRankChanges(
    ...
    // 이전 기간 키워드 순위 맵 생성
    Map<String, Integer> previousRankMap = previousKeywords.stream()
        .collect(Collectors.toMap(
            KeywordStats::getKeyword,
            KeywordStats::getRank,
