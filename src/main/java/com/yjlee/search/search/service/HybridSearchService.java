...
public class HybridSearchService {
  ...
  public SearchExecuteResponse hybridSearch(
    ...
    log.info("Starting hybrid search - query: {}, mode: {}, rrfK: {}, topK: {}",
        request.getQuery(),
    ...
    try {
      ...
      CompletableFuture<List<Hit<JsonNode>>> bm25Future = CompletableFuture.supplyAsync(() ->
          executeBM25Search(indexName, request, request.getHybridTopK())
      );
      
      CompletableFuture<List<Hit<JsonNode>>> vectorFuture = CompletableFuture.supplyAsync(() ->
      ...
      log.debug("Search results - BM25: {} hits, Vector: {} hits", 
          bm25Results.size(), vectorResults.size());
      
      // 2. RRF 병합
      List<RRFScorer.RRFResult> mergedResults = rrfScorer.mergeWithRRF(
          bm25Results,
          vectorResults,
          request.getRrfK(),
          request.getSize()
      ...
      SearchExecuteResponse response = buildHybridResponse(
          request,
          mergedResults,
          bm25Results.size(),
          vectorResults.size(),
      ...
      log.info("Hybrid search completed in {}ms - final results: {}", 
          took, mergedResults.size());
  ...
  private List<Hit<JsonNode>> executeBM25Search(
    ...
    try {
      ...
      SearchRequest searchRequest = SearchRequest.of(s -> s
          ...
          .size(topK)
          .source(src -> src
              .filter(f -> f.excludes("name_specs_vector"))
          )
      );
  ...
  private SearchExecuteResponse buildHybridResponse(
    ...
    for (int i = 0; i < mergedResults.size(); i++) {
      ...
      ProductDto.ProductDtoBuilder productBuilder = ProductDto.builder()
          .id(result.getDocument().id())
          .score(result.getTotalRrfScore());
      
      if (source.has("name")) {
        productBuilder.name(source.get("name").asText());
        productBuilder.nameRaw(source.has("name_raw") ? source.get("name_raw").asText() : source.get("name").asText());
      }
      if (source.has("model")) {
      ...
      if (source.has("specs")) {
        ...
        productBuilder.specsRaw(source.has("specs_raw") ? source.get("specs_raw").asText() : source.get("specs").asText());
      }
