...
public class SearchRequestMapper {
  ...
  private boolean hasFilters(SearchParams params) {
    return params.getBrand() != null 
        || params.getCategory() != null 
        || hasPriceFilter(params);
  }
  ...
  private void copyCommonFields(SearchParams params, SearchExecuteRequest request) {
    ...
    // 검색 모드 관련
    request.setSearchMode(Optional.ofNullable(params.getSearchMode()).orElse(SearchMode.KEYWORD_ONLY));
    request.setRrfK(Optional.ofNullable(params.getRrfK()).orElse(SearchConstants.DEFAULT_RRF_K));
    request.setHybridTopK(Optional.ofNullable(params.getHybridTopK()).orElse(SearchConstants.DEFAULT_HYBRID_TOP_K));
