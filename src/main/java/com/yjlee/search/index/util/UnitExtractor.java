...
public class UnitExtractor {
  ...
  private static final String KO_UNITS = 
      "개입|개|장|매|봉|봉지|포|박스|팩|세트|켤레|족|쌍|병|캔|정|알|평|묶음|다발";

  // 일반 단위 패턴 (숫자 + 단위)
  private static final Pattern UNIT_PATTERN_EN = Pattern.compile(
      "(\\d+\\.?\\d*)\\s*(" + EN_UNITS + ")(?![a-zA-Z])",
      Pattern.CASE_INSENSITIVE
  );
  
  private static final Pattern UNIT_PATTERN_KO = Pattern.compile(
      "(\\d+\\.?\\d*)\\s*(" + KO_UNITS + ")(?![가-힣])"
  );
  
  // 복합 패턴 (예: 25.4x24.4cm, 1920x1080px)
  private static final Pattern COMPLEX_UNIT_PATTERN = Pattern.compile(
