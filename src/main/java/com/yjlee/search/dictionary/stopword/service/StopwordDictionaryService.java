...
public class StopwordDictionaryService implements DictionaryService {
  ...
  public StopwordDictionaryResponse create(StopwordDictionaryCreateRequest request, DictionaryEnvironmentType environment) {
    StopwordDictionary entity = StopwordDictionary.of(request.getKeyword(), request.getDescription(), environment);
  ...
  public PageResponse<StopwordDictionaryListResponse> getList(
    ...
    Page<StopwordDictionary> entities = (keyword != null && !keyword.trim().isEmpty())
  ...
  public StopwordDictionaryResponse get(Long id, DictionaryEnvironmentType environment) {
    ...
    StopwordDictionary entity = stopwordDictionaryRepository.findById(id)
        .orElseThrow(() -> new EntityNotFoundException("불용어 사전을 찾을 수 없습니다: " + id));
    return mapper.toResponse(entity);
  }

  public StopwordDictionaryResponse update(Long id, StopwordDictionaryUpdateRequest request, DictionaryEnvironmentType environment) {
  ...
  private void deployToEnvironment(DictionaryEnvironmentType from, DictionaryEnvironmentType to) {
    ...
    List<StopwordDictionary> targetDictionaries = sourceDictionaries.stream()
  ...
  public void deployToEC2(String version) {
    ...
    try {
      ...
      if (content == null || content.trim().isEmpty()) {
        log.warn("불용어사전 내용이 비어있음 - 버전: {}", version);
        return;
      }
      
      EC2DeploymentService.EC2DeploymentResult result = 
          ec2DeploymentService.deployFile(version + ".txt", DeploymentConstants.EC2Paths.STOPWORD_DICT, content, version);
  ...
  private Page<StopwordDictionary> searchInRepository(DictionaryEnvironmentType environmentType, String keyword, Pageable pageable) {
    return stopwordDictionaryRepository.findByEnvironmentTypeAndKeywordContainingIgnoreCase(environmentType, keyword, pageable);
  }
