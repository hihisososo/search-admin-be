...
public class UnitDictionaryService implements DictionaryService {
  ...
  public UnitDictionaryResponse create(UnitDictionaryCreateRequest request, DictionaryEnvironmentType environment) {
  ...
  public PageResponse<UnitDictionaryListResponse> getList(
    ...
    Page<UnitDictionary> entities = (keyword != null && !keyword.trim().isEmpty())
  ...
  public UnitDictionaryResponse get(Long id, DictionaryEnvironmentType environment) {
    ...
    UnitDictionary entity = unitDictionaryRepository.findById(id)
        .orElseThrow(() -> new EntityNotFoundException("단위 사전을 찾을 수 없습니다: " + id));
    return mapper.toResponse(entity);
  }

  public UnitDictionaryResponse update(Long id, UnitDictionaryUpdateRequest request, DictionaryEnvironmentType environment) {
  ...
  private void deployToEnvironment(DictionaryEnvironmentType from, DictionaryEnvironmentType to) {
    ...
    List<UnitDictionary> targetDictionaries = sourceDictionaries.stream()
  ...
  public void deployToEC2(String version) {
    ...
    try {
      ...
      EC2DeploymentService.EC2DeploymentResult result = 
          ec2DeploymentService.deployFile(version + ".txt", DeploymentConstants.EC2Paths.UNIT_DICT, content, version);
  ...
  private Page<UnitDictionary> searchInRepository(DictionaryEnvironmentType environmentType, String keyword, Pageable pageable) {
    return unitDictionaryRepository.findByEnvironmentTypeAndKeywordContainingIgnoreCase(environmentType, keyword, pageable);
  }
