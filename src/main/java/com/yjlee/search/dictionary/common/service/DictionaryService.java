...
/**
 * 사전 서비스 공통 인터페이스
 * 각 사전은 이 인터페이스를 구현하여 배포 및 동기화 기능을 제공
 */
public interface DictionaryService {

  /**
   * 개발 환경으로 배포 (CURRENT → DEV)
   * 내부적으로 EC2 업로드, 캐시 업데이트, ES 동기화 등을 처리
   ...
   */
  void deleteByEnvironmentType(DictionaryEnvironmentType environment);

  /**
   * 실시간 동기화 (선택적)
   * 캐시 업데이트, ES synonym set 업데이트 등
