...
public class SynonymDictionaryService {
  ...
  public SynonymDictionaryResponse create(
    ...
    SynonymDictionary dictionary = SynonymDictionary.of(request.getKeyword(), environment);
  ...
  public void upload(List<SynonymDictionary> synonyms, String version) {
  ...
  public void sync(List<SynonymDictionary> synonyms, String synonymSetName) {
    try {
      List<String> synonymRules =
          synonyms.stream().map(SynonymDictionary::getKeyword).collect(Collectors.toList());

      updateElasticsearchSynonymSet(synonymSetName, synonymRules);
      log.info("동의어 세트 동기화 완료 - set: {}, 규칙 수: {}", synonymSetName, synonymRules.size());
    } catch (Exception e) {
      log.error("동의어 세트 동기화 실패", e);
      throw new RuntimeException("동의어 세트 동기화 실패", e);
    }
  }
