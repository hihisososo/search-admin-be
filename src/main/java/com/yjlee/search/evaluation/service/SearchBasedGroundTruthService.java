...
public class SearchBasedGroundTruthService {
  ...
  private void processQueries(
    ...
    queries.parallelStream().forEach(query -> {
      ...
      try {
        ...
        // 300개 제한
        Map<String, String> limitedCandidates = candidatesWithSource.entrySet().stream()
        ...
        limitedCandidates.forEach((productId, searchSource) -> {
          ProductDocument product = productMap.get(productId);
          QueryProductMapping mapping = QueryProductMapping.builder()
        ...
        log.debug("쿼리 '{}' 처리 완료: 총 {}개 후보 중 {}개 저장",
            query.getQuery(), candidatesWithSource.size(), limitedCandidates.size());
    ...
    log.info("{} 쿼리 정답 후보군 생성 완료: {}개 쿼리, {}개 매핑", 
        processType, queries.size(), mappings.size());
  }
  ...
  private Map<String, String> collectCandidatesWithSourceTracking(
    ...
    // 바이그램 검색
    String[] bigramFields = {
        "name_candidate.bigram", 
        "specs_candidate.bigram", 
        "category_candidate.bigram"
    };
    searchByCrossField(query, bigramFields).forEach(id -> {
      if (!productSourceMap.containsKey(id)) {
  ...
  private List<String> searchByVector(float[] embedding) {
    return searchByVector(embedding, FIXED_PER_STRATEGY, FIXED_VECTOR_NUM_CANDIDATES, vectorMinScore);
  }
  ...
  private Map<String, ProductDocument> fetchProductsBulk(Set<String> productIds) {
    ...
    try {
      ...
      var mgetResponse = elasticsearchClient.mget(
          m -> m.index(indexName).ids(new ArrayList<>(productIds)), 
      ...
      log.debug("Bulk fetch 완료: 요청 {}개, 조회 성공 {}개", 
          productIds.size(), productMap.size());
