...
class SynonymDictionaryControllerTest {
  ...
  void setUp() {
    ...
    // 테스트용 유의어 사전 데이터 생성
    SynonymDictionary dict1 =
        SynonymDictionary.builder().keyword("노트북,랩탑").description("컴퓨터 동의어").build();

    SynonymDictionary dict2 =
        SynonymDictionary.builder().keyword("휴대폰,핸드폰,스마트폰").description("모바일 기기 동의어").build();

    SynonymDictionary dict3 =
        SynonymDictionary.builder().keyword("자동차,차량,승용차").description("교통수단 동의어").build();
  ...
  void getSynonymDictionaryDetail() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/dictionaries/synonym/{id}", firstDict.getId()))
        .andDo(print())
        .andExpect(status().isOk())
  ...
  void createSynonymDictionary() throws Exception {
    ...
    // DB에 실제로 저장되었는지 확인
    mockMvc
        .perform(get("/api/v1/dictionaries/synonym").param("search", "TV"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.content", hasSize(1)))
  ...
  void updateSynonymDictionary() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/v1/dictionaries/synonym/{id}", firstDict.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
        .andDo(print())
        .andExpect(status().isOk())
  ...
  void deleteSynonymDictionary() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/v1/dictionaries/synonym/{id}", deletedId))
        .andDo(print())
        .andExpect(status().isNoContent());

    // 삭제 후 조회 시 404 에러 확인
    mockMvc
        .perform(get("/api/v1/dictionaries/synonym/{id}", deletedId))
  ...
  void getSynonymDictionariesPaging() throws Exception {
    ...
    for (int i = 1; i <= 10; i++) {
      SynonymDictionary dict =
          SynonymDictionary.builder().keyword("동의어" + i).description("테스트 동의어 " + i).build();
      synonymDictionaryRepository.save(dict);
    }

    // 첫 페이지 조회 (5개)
    mockMvc
        .perform(get("/api/v1/dictionaries/synonym").param("page", "1").param("size", "5"))
        .andDo(print())
        .andExpect(status().isOk())
