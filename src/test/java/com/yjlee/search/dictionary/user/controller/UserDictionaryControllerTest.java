...
class UserDictionaryControllerTest {
  ...
  void setUp() {
    ...
    // 테스트용 사용자 사전 데이터 생성
    UserDictionary dict1 = UserDictionary.builder()
        .keyword("LG그램")
        .description("LG 노트북 브랜드")
        .build();
    
    UserDictionary dict2 = UserDictionary.builder()
        .keyword("갤럭시북")
        .description("삼성 노트북 브랜드")
        .build();
    
    UserDictionary dict3 = UserDictionary.builder()
        .keyword("맥북프로")
        .description("애플 노트북 브랜드")
        .build();
  ...
  void getUserDictionaries() throws Exception {
    mockMvc.perform(get("/api/v1/dictionaries/user")
            .param("page", "1")
            .param("size", "20")
  ...
  void getUserDictionaryDetail() throws Exception {
    ...
    mockMvc.perform(get("/api/v1/dictionaries/user/{id}", firstDict.getId()))
        .andDo(print())
        .andExpect(status().isOk())
  ...
  void createUserDictionary() throws Exception {
    UserDictionaryCreateRequest request = UserDictionaryCreateRequest.builder()
        .keyword("아이패드프로")
        .description("애플 태블릿 브랜드")
        .build();

    mockMvc.perform(post("/api/v1/dictionaries/user")
  ...
  void updateUserDictionary() throws Exception {
    ...
    UserDictionaryUpdateRequest request = UserDictionaryUpdateRequest.builder()
        .keyword("LG그램17")
        .description("LG 17인치 노트북")
        .build();

    mockMvc.perform(put("/api/v1/dictionaries/user/{id}", firstDict.getId())
            .contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(request)))
        .andDo(print())
        .andExpect(status().isOk())
  ...
  void deleteUserDictionary() throws Exception {
    ...
    mockMvc.perform(delete("/api/v1/dictionaries/user/{id}", deletedId))
        .andDo(print())
        .andExpect(status().isNoContent());
    
    // 삭제 후 조회 시 404 에러 확인
    mockMvc.perform(get("/api/v1/dictionaries/user/{id}", deletedId))
  ...
  void getUserDictionaries_Paging() throws Exception {
    // 추가 데이터 생성
    for (int i = 1; i <= 10; i++) {
      UserDictionary dict = UserDictionary.builder()
          .keyword("제품명" + i)
          .description("테스트 제품 " + i)
          .build();
