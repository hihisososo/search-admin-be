...
-- "ㄴㅌㅂ " 과 같이 초성으로도 검색
-- "노트ㅂ" 과 같이 자소단위 일치 검색
-- "삼성노트북" 과 같이 띄어쓰기 불일치 경우에도 검색
-- "노트북삼성" 과 같은 순서가 뒤바뀐 검색어도 검색
- 구현
-- 초성검색 처리를 위해 초성 변환필드 추가
-- 자소검색 처리를 위해 자소 변환필드 추가
-- 띄어쓰기 불일치 처리를 위해 공백제거 필드 추가
-- 순서가 뒤바뀐 검색어 처리를 위해 형태소 분석 필드 추가
...
- 기능 정의
-- 상품명과 스펙(상세설명) 에서 검색 되어야 함
-- 상품명 에서 일치했을 경우, 우선순위가 높아야 함
-- 모델명은 일부만 검색해도 검색되어야 함
- 구현
-- 상품명, 스펙 각각 형태소분석, 바이그램 필드 추가
-- 모델명은 추가 추출하여 바이그램 필드로 추가
- 검색 우선순위
-- 1순위 : 상품명(형태소분석), 스펙(형태소분석), 모델명(바이그램) 을 cross_field 검색
-- 2순위 : 상품명(바이그램) + 스펙(바이그램) 을 cross_field 검색
-- 검색어에서 카테고리, 브랜드 일치할 경우 추가 부스팅
- Elasticsearch 필드 설정
